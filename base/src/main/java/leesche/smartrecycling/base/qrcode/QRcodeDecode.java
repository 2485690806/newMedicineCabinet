...
public class QRcodeDecode {
    ...
    // 图像处理后的图片存放地址
    private static String PATH = Constants.BASE_CACHE_DIR + "/test.jpg";
...
//		decode("C:\\Users\\guanh\\Desktop\\vouchers");
     ...
     */
    public static List<String> decode(String directoryPath) {
        // 需要进行识别的图片所在文件夹路径
         ...
         */
        String qRcode = decodeQRcode(detector, directoryPath);
        if (qRcode == null || "".equals(qRcode)) {
            // 对图像进行处理，定位图像中的二维码，将其截取出来
            findQRcodeAndCut(directoryPath);
...
//	}
    ...
    public static String decodeQRcode(QRCodeDetector detector, String qrCodePath) {
        ...
        try {
            File qrCodeFile = new File(qrCodePath); // qrCodePath 是图片文件路径
            Bitmap bitmap = BitmapFactory.decodeFile(qrCodeFile.getAbsolutePath());
            if (bitmap == null) {
                Logger.e("Failed to load original image: " +qrCodePath);
                return null;
            }
            // 获取 Bitmap 的宽度和高度
            ...
            // 存储 Bitmap 的像素数据（格式为 ARGB_8888，每个像素占 4 字节）
            int[] pixels = new int[width * height];
            // 将 Bitmap 的像素数据复制到数组中
            bitmap.getPixels(pixels, 0, width, 0, 0, width, height);

            // 使用 RGBLuminanceSource 构造 LuminanceSource（替代 BufferedImageLuminanceSource）
            // 注意：RGBLuminanceSource 会自动从 ARGB 像素中提取亮度信息（Y = 0.299R + 0.587G + 0.114B）
