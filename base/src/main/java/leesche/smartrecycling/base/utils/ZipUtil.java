...
//                Logger.i("【解压保存】文件路径：" + outPathString + File.separator + szName);
        ...
        inZip.close();
        //删除ZIP文件
        File file = new File(Constants.BASE_CACHE_DIR);
        File[] files = file.listFiles();
        for (int i = 0; i < files.length; i++) {
            if(files[i].getName().contains("zip")){
                files[i].delete();
//                Logger.i("【解压删除】文件路径：" + files[i].getAbsolutePath());
            }
        }
        SoundPoolHelper.getInstance2(context).resetSoundPoolHelper(context);//释放并加载语言文件
    }
...
//                Logger.e("【解压】" + outPathString + File.separator + szName);
                File file = new File(outPathString + File.separator + szName);
                if (!file.exists()) {
//                    Logger.e("【解压】" + "Create the file:" + outPathString + File.separator + szName);
                    file.getParentFile().mkdirs();
                    file.createNewFile();
                }
                // 获取文件的输出流
    ...
    public static void zipFolder(String srcFileString, String zipFileString) throws Exception {
        ...
        //压缩
//        Logger.i("【压缩】" + "---->" + file.getParent() + "===" + file.getAbsolutePath());
...
//        Logger.i("【压缩】" + "folderString:" + folderString + "\n" +
