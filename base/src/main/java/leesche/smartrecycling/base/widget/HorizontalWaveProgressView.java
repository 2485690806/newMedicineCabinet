...
import leesche.smartrecycling.base.utils.DensityUtil;
...
public class HorizontalWaveProgressView extends View {
    ...
    public HorizontalWaveProgressView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        waveLength = typedArray.getDimension(R.styleable.HorizontalWaveProgressView_wave_length, DensityUtil.dp2px(context, 25));
        //获取波浪高度
        waveHeight = typedArray.getDimension(R.styleable.HorizontalWaveProgressView_wave_height, DensityUtil.dp2px(context, 5));
        //获取波浪颜色
    ...
    private void init(Context context) {
        //设置自定义View的宽高
        waveDefaultWidth = DensityUtil.dp2px(context, 152);
        waveDefaultHeight = DensityUtil.dp2px(context, 40);
        dp1 = DensityUtil.dp2px(getContext(), 1);
        dp27 = DensityUtil.dp2px(getContext(), 15);
    ...
    private Path canvasWavePath() {
        ...
        wavePath.moveTo(-moveDistance, (1 - currentPercent) * waveActualSizeHeight);
        // 最多能绘制多少个波浪
    ...
    private Path canvasSecondPath() {
        ...
        //移动到右上方，也就是p1点
        secondWavePath.moveTo(-waveActualSizeHeight - moveDistance, (1 - currentPercent) * waveActualSizeWidth);
        //p1 - p0
        for (int i = 0; i < waveNumber * 2; i++) {
            secondWavePath.rQuadTo(-waveLength / 2, waveHeight, -waveLength, 0);
            secondWavePath.rQuadTo(-waveLength / 2, -waveHeight, -waveLength, 0);
        }
        //p3-p0的高度随着进度变化而变化
        ...
        secondWavePath.lineTo(waveActualSizeWidth, waveActualSizeHeight);
        //连接p2 - p1
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        waveNumber = (int) Math.ceil(Double.parseDouble(String.valueOf(waveActualSizeWidth / waveLength / 2)));
    }
