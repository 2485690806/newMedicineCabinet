        ...
        private void initWebSocket(final ObservableEmitter<WebSocketInfo> emitter) {
            webSocket = client.newWebSocket(getRequest(url), new WebSocketListener() {
                ...
                public void onOpen(final WebSocket webSocket, Response response) {
                    if (showLog) {
                        Log.i("【WebSocket】", "WebSocket open success");
                    }
                ...
                public void onFailure(WebSocket webSocket, Throwable t, Response response) {
                    if (showLog) {
                        Log.e("【WebSocket】", "WebSocket onFailure: " + webSocket.request().url().uri().getPath());
                    }
                    if (!emitter.isDisposed()) {
                        emitter.onError(t);
                    }
...
//                    webSocket.close(1000, null);
                    ...
                    Log.i("【WebSocket】", "WebSocket onClosing: " + code + " " + reason);
                    if (!emitter.isDisposed()) {
                ...
                public void onClosed(WebSocket webSocket, int code, String reason) {
                    Log.i("【WebSocket】", "WebSocket onClosed: " + code + " " + reason);
                    if (!emitter.isDisposed()) {
                        emitter.onNext(new WebSocketInfo(true));
                    }
                }
            ...
            emitter.setCancellable(new Cancellable() {
                ...
                public void cancel() throws Exception {
                    ...
                    if (showLog) {
                        Log.i("【WebSocket】", "WebSocket cancel: " + url);
                    }
                }
            });
        }
