...
//                    }
    ...
    /**
     * 根据VideoType更新TextView内容并处理自动切换
     */
    public void setVideoType(VideoType type) {
        ...
        switch (type) {
            case IDLE:
                lqTextView.setText("扫码领取你的免费瓶装水");
                break;

            case START_DO_WATER:
                lqTextView.setText("制水中...");
                // 120秒后自动切换到IDLE
                idleSwitchRunnable = () -> setVideoType(VideoType.IDLE);
                handler.postDelayed(idleSwitchRunnable, 120 * 1000);
                break;

            case FINISH_DO_WATER:
                lqTextView.setText("制水完成");
                // 5秒后自动切换到IDLE
                idleSwitchRunnable = () -> setVideoType(VideoType.IDLE);
